...
func (pq *PatientQuery) QueryMedicalcare() *MedicalCareQuery {
	query := &MedicalCareQuery{config: pq.config}
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := pq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (pq *PatientQuery) QueryNurse() *NurseQuery {
...
func (pq *PatientQuery) QueryDisease() *DiseaseQuery {
	query := &DiseaseQuery{config: pq.config}
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := pq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (pq *PatientQuery) QueryMedicalfiles() *MedicalfileQuery {
	query := &MedicalfileQuery{config: pq.config}
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := pq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (pq *PatientQuery) QueryQueue() *QueueQuery {
	query := &QueueQuery{config: pq.config}
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := pq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (pq *PatientQuery) QueryAppointment() *AppointmentQuery {
	query := &AppointmentQuery{config: pq.config}
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := pq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (pq *PatientQuery) WithMedicalcare(opts ...func(*MedicalCareQuery)) *PatientQuery {
	query := &MedicalCareQuery{config: pq.config}
	for _, opt := range opts {
		opt(query)
	}
	pq.withMedicalcare = query
	return pq
}
...
func (pq *PatientQuery) WithNurse(opts ...func(*NurseQuery)) *PatientQuery {
	query := &NurseQuery{config: pq.config}
	for _, opt := range opts {
		opt(query)
	}
...
func (pq *PatientQuery) WithDisease(opts ...func(*DiseaseQuery)) *PatientQuery {
	query := &DiseaseQuery{config: pq.config}
	for _, opt := range opts {
		opt(query)
	}
	pq.withDisease = query
	return pq
}
...
func (pq *PatientQuery) WithMedicalfiles(opts ...func(*MedicalfileQuery)) *PatientQuery {
	query := &MedicalfileQuery{config: pq.config}
	for _, opt := range opts {
		opt(query)
	}
	pq.withMedicalfiles = query
	return pq
}
...
func (pq *PatientQuery) WithQueue(opts ...func(*QueueQuery)) *PatientQuery {
	query := &QueueQuery{config: pq.config}
	for _, opt := range opts {
		opt(query)
	}
...
func (pq *PatientQuery) WithAppointment(opts ...func(*AppointmentQuery)) *PatientQuery {
	query := &AppointmentQuery{config: pq.config}
	for _, opt := range opts {
		opt(query)
	}
...
//		GroupBy(patient.FieldPatientID).
//		Aggregate(ent.Count()).
//		Scan(ctx, &v)
//
...
func (pq *PatientQuery) sqlAll(ctx context.Context) ([]*Patient, error) {
	var (
		...
		loadedTypes = [7]bool{
			pq.withGender != nil,
			pq.withMedicalcare != nil,
			pq.withNurse != nil,
			pq.withDisease != nil,
			pq.withMedicalfiles != nil,
			pq.withQueue != nil,
			pq.withAppointment != nil,
		}
	)
	if pq.withGender != nil || pq.withMedicalcare != nil || pq.withNurse != nil || pq.withDisease != nil {
		withFKs = true
	}
	if withFKs {
	...
	if query := pq.withMedicalcare; query != nil {
		ids := make([]int, 0, len(nodes))
		nodeids := make(map[int][]*Patient)
		for i := range nodes {
	...
	if query := pq.withDisease; query != nil {
		ids := make([]int, 0, len(nodes))
		nodeids := make(map[int][]*Patient)
		for i := range nodes {
	...
	if query := pq.withMedicalfiles; query != nil {
		...
		query.Where(predicate.Medicalfile(func(s *sql.Selector) {
			s.Where(sql.InValues(patient.MedicalfilesColumn, fks...))
		}))
		neighbors, err := query.All(ctx)
		if err != nil {
			return nil, err
	...
	if query := pq.withQueue; query != nil {
		fks := make([]driver.Value, 0, len(nodes))
		nodeids := make(map[int]*Patient)
		for i := range nodes {
			fks = append(fks, nodes[i].ID)
		...
		query.Where(predicate.Queue(func(s *sql.Selector) {
			s.Where(sql.InValues(patient.QueueColumn, fks...))
		}))
		neighbors, err := query.All(ctx)
		if err != nil {
			return nil, err
	...
	if query := pq.withAppointment; query != nil {
		...
		query.Where(predicate.Appointment(func(s *sql.Selector) {
			s.Where(sql.InValues(patient.AppointmentColumn, fks...))
		}))
		neighbors, err := query.All(ctx)
		if err != nil {
			return nil, err
