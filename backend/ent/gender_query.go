...
func (gq *GenderQuery) QueryPatients() *PatientQuery {
...
func (gq *GenderQuery) QueryDentists() *DentistQuery {
	query := &DentistQuery{config: gq.config}
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := gq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (gq *GenderQuery) WithPatients(opts ...func(*PatientQuery)) *GenderQuery {
	query := &PatientQuery{config: gq.config}
	for _, opt := range opts {
		opt(query)
	}
	gq.withPatients = query
	return gq
}
...
func (gq *GenderQuery) WithDentists(opts ...func(*DentistQuery)) *GenderQuery {
	query := &DentistQuery{config: gq.config}
	for _, opt := range opts {
		opt(query)
	}
	gq.withDentists = query
	return gq
}
...
func (gq *GenderQuery) sqlAll(ctx context.Context) ([]*Gender, error) {
	...
	if query := gq.withPatients; query != nil {
		...
		query.Where(predicate.Patient(func(s *sql.Selector) {
			s.Where(sql.InValues(gender.PatientsColumn, fks...))
		}))
	...
	if query := gq.withDentists; query != nil {
		fks := make([]driver.Value, 0, len(nodes))
		nodeids := make(map[int]*Gender)
		for i := range nodes {
			fks = append(fks, nodes[i].ID)
		...
		query.Where(predicate.Dentist(func(s *sql.Selector) {
			s.Where(sql.InValues(gender.DentistsColumn, fks...))
		}))
		neighbors, err := query.All(ctx)
		if err != nil {
			return nil, err
