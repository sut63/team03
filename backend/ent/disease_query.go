...
func (dq *DiseaseQuery) QueryPatients() *PatientQuery {
...
func (dq *DiseaseQuery) WithPatients(opts ...func(*PatientQuery)) *DiseaseQuery {
	query := &PatientQuery{config: dq.config}
	for _, opt := range opts {
		opt(query)
	}
	dq.withPatients = query
	return dq
}
...
func (dq *DiseaseQuery) sqlAll(ctx context.Context) ([]*Disease, error) {
	...
	if query := dq.withPatients; query != nil {
		...
		query.Where(predicate.Patient(func(s *sql.Selector) {
			s.Where(sql.InValues(disease.PatientsColumn, fks...))
		}))
