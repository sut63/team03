...
func (rq *RoomQuery) QueryAppointment() *AppointmentQuery {
	query := &AppointmentQuery{config: rq.config}
...
func (rq *RoomQuery) WithAppointment(opts ...func(*AppointmentQuery)) *RoomQuery {
	query := &AppointmentQuery{config: rq.config}
	for _, opt := range opts {
		opt(query)
	}
	rq.withAppointment = query
	return rq
}
...
func (rq *RoomQuery) sqlAll(ctx context.Context) ([]*Room, error) {
	...
	if query := rq.withAppointment; query != nil {
		...
		query.Where(predicate.Appointment(func(s *sql.Selector) {
			s.Where(sql.InValues(room.AppointmentColumn, fks...))
		}))
